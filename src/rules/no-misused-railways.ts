    ...
    schema: [
      {
        ...
        properties: {
            ...
            oneOf: [
              { type: 'boolean' },
              {
                ...
                properties: {
                  arguments: {
                    description:
                      'Disables checking result-returning function passed as argument where the parameter type expects a function that returns `void`.',
                    type: 'boolean',
                  },
                  attributes: {
                    description:
                      'Disables checking result-returning function passed as a JSX attribute expected to be a function that returns `void`.',
                    type: 'boolean',
                  },
                  inheritedMethods: {
                    description:
                      'Disables checking result-returning method in a type that extends or implements another type expecting that method to return `void`.',
                    type: 'boolean',
                  },
                  properties: {
                    description:
                      'Disables checking result-returning function passed as an object property expected to be a function that returns `void`.',
                    type: 'boolean',
                  },
                  returns: {
                    description:
                      'Disables checking result-returning function returned in a function whose return type is a function that returns `void`.',
                    type: 'boolean',
                  },
                  variables: {
                    description:
                      'Disables checking result-returning function used as a variable whose return type is a function that returns `void`.',
                    type: 'boolean',
                  },
                },
                type: 'object',
          ...
          checksSpreads: {
            description: 'Whether to warn when `...` spreading a `Result`.',
            type: 'boolean',
          },
        },
      },
  ...
  create(context, [{ checksConditionals, checksVoidReturn, checksSpreads }]) {
    ...
    function checkReturnStatement(node: TSESTree.ReturnStatement): void {
      ...
      const functionNode = (() => {
        ...
        while (current && !util.isFunction(current)) {
          current = current.parent
        }
        return util.nullThrows(current, util.NullThrowsReasons.MissingParent)
      })()

      if (functionNode.returnType && !isPossiblyFunctionType(functionNode.returnType)) {
        return
      }
    ...
    function checkClassLikeOrInterfaceNode(
      ...
      for (const nodeMember of tsNode.members) {
        ...
        if (!returnsResult(checker, nodeMember)) {
          continue
        }
     ...
     * @param nodeMember Node member that returns a Result
  ...
  for (const subType of tsutils.unionTypeParts(type)) {
    ...
    for (const signature of signatures) {
      for (const [index, parameter] of signature.parameters.entries()) {
        ...
        if (decl && util.isRestParameterDeclaration(decl)) {
...
function returnsResult(checker: ts.TypeChecker, node: ts.Node): boolean {
  ...
  return tsutils
    .unionTypeParts(type)
    .some((t) => anySignatureIsResultType(checker, node, t))
}
 ...
 */
function getMemberIfExists(type: ts.Type, memberName: string): ts.Symbol | undefined {
